...
export default function ResetPassword() {
    ...
    useEffect(() => {
        // Retrieve token and email from URL matching what Laravel usually sends
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSuccess(response.data.message || t('auth.password_reset_success'));
            setTimeout(() => {
                navigate('/login');
            }, 3000);
        } catch (err) {
            setError(err.response?.data?.message || t('auth.password_reset_failed'));
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="min-h-screen flex bg-gray-50">
            ...
            <div className="hidden lg:flex w-1/2 bg-blue-600 relative overflow-hidden items-center justify-center">
                ...
                <div className="relative z-10 text-white p-12 max-w-lg text-center animate-fade-in">
                    <h1 className="text-5xl font-extrabold mb-6 tracking-tight">{t('auth.secure_account_title')}</h1>
                    <p className="text-xl text-blue-100 font-light leading-relaxed">
                        {t('auth.secure_account_desc')}
                    </p>
                </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12">
                <div className="w-full max-w-md space-y-8 animate-slide-in-right">
                    <div className="text-center lg:text-left">
                        <h2 className="text-4xl font-extrabold text-gray-900 mb-2">{t('auth.create_new_password_title')}</h2>
                        <p className="text-gray-500">{t('auth.new_password_subtitle')}</p>
                    </div>
                    ...
                    <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                        {success && (
                            <div className="bg-green-50 border-l-4 border-green-500 text-green-700 p-4 rounded-md shadow-sm animate-fade-in flex items-center">
                                ...
                                <span className="text-sm font-medium">{success} {t('auth.redirecting_login')}</span>
                            </div>
                        )}
                        ...
                        <div className="space-y-5">
                            ...
                            <div>
                                ...
                                <input
                                    type="email"
                                    required
                                    className="appearance-none block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-gray-50"
                                    value={email}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">{t('auth.confirm_new_password')}</label>
                                <input
                                    type="password"
                                    required
                                    className="appearance-none block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                        ...
                        <div>
                            <button
                                ...
                                className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white transition-all duration-200 ease-in-out transform ${(isLoading || !!success || !token)
